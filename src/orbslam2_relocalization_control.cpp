...
class ORBSLAM2_Control
{
  ...
  private:
    ...
    std::atomic<int> goback_trigger_data_{0};//bool to identify the time to go back, shared between threads
    std::atomic<int> relocalization_trigger_data_{0};//bool to trigger relocalization, shared between threads
...
//---------------------------- Topic callbacks ---------------------------//
    ...
    void navigateBackCb(const std_msgs::bool::ConstPtr& msg)
    {
